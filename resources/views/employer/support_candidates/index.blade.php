...
@section('title', request()->getHost() . ': ' . __('suitable candidate'))
            ...
            <table class="table">
                ...
                <tbody class="table-group-divider" id="result">
                    @foreach ($supportCandidates as $supportCandidate)
                        <tr @if(!$supportCandidate->is_read) class="table-warning" @endif >
                            <th scope="row">
                            ...
                            <td>
                                <strong>
                                    {{ $supportCandidate->job->name . ' #' . $supportCandidate->id }}
                                    @if (!$supportCandidate->is_read)
                                    ...
                                    <ul class="dropdown-menu">
                                        @foreach ($listStatus as $status)
                                            @if ($status != 'all')
                                                <li>
                                                    <form
                                                        action="{{ route('employer.support_candidates.update', $supportCandidate->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('put')
...
@php
    App\Models\SupportCandidate::whereIn('id', $supportCandidates->pluck('id'))->update(['is_read' => true]);
@endphp